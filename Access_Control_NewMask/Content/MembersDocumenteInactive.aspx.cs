...
{
    public partial class MembersDocumenteInactive : BasePage
    {
        ...
        [WebMethod]
        public static void SaveMemberDocuments(string jSONData)
        {
            List<MemberDocumentsDto> memberData = JsonConvert.DeserializeObject<List<MemberDocumentsDto>>(jSONData);
            if (memberData.Count > 0)
            {
                var member = new MembersDataRepository().GetMemberDataById(Convert.ToInt64(memberData[0].MemberID));
                if (member == null) return;
                new MembersDocumentsViewModel().saveMemberDocuments(memberData[0]);
            }
