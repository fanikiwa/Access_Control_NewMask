...
{
    public partial class PersonalDocumente : BasePage
    {
            ...
            if (HttpContext.Current.Session["PageOrigin"] != null)
            {
                switch (HttpContext.Current.Session["PageOrigin"].ToString())
                {
                    case PERSONAL_PAGE_ORIGIN:
                        ...
                        redirectURL = string.Format("Personal_New.aspx?{0}={1}", PERSONAL_QUERY_PARAM_KEY, personalNumberEncrypted);
                        break;
                    default:
            ...
            if (Session["PageOrigin"] != null)
            {
                switch (Session["PageOrigin"].ToString())
                {
                    case PERSONAL_PAGE_ORIGIN:
                        ...
                        redirectURL = string.Format("Personal.aspx?{0}={1}", PERSONAL_QUERY_PARAM_KEY, personalNumberEncrypted);
                        break;
                    case "7E30B050":
                        redirectURL = "PersonalForm.aspx?PersIDNr=" + Session[PERSONAL_QUERY_PARAM_KEY].ToString();
                        break;
                    default:
                        break;
                }
