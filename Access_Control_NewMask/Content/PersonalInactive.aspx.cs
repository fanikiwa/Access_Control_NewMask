...
{
    public partial class PersonalInactive : BasePage
    {
        #region Properties
        ...
        public static List<PersonnelDto> PersonnelList
        {
            get
            {
                return new PersonalViewModel().GetAllInActivePersonnel();
            }
            set
            {
                new PersonalViewModel().GetAllInActivePersonnel();
            }
        }
        #endregion
        ...
        protected void Page_Init(object sender, EventArgs e)
        {
            ...
            {
                btnActivatePersonal.Enabled = false; btnSaveAusweis.Enabled = false; btnSaveCarType.Enabled = false; btnSaveClient.Enabled = false; btnSavePersType.Enabled = false;
        ...
        {
            this.Form.DefaultButton = this.btnActivatePersonal.UniqueID;
            if (!IsPostBack)
            {
                ...
                Debug.WriteLine(id);
                int personid = 0;
                bool isidint = int.TryParse(id, out personid);
                if (isidint) { } else { return; }

                if (!string.IsNullOrEmpty(id))
                {
                    if (cmbIDNr.Items.FindByValue(id.ToString()) != null)
                    {
                        cmbIDNr.Value = id.ToString();
                        cmbPersName.Value = id.ToString();
                        cmbAusweisNr.Value = id.ToString();
                    }
                }

                if (Convert.ToInt64(cmbIDNr.Value) > 0)
                {
                    //LoadFirstPersonnel();
                }
        ...
        {
            List<PersAccessGroupsDto> personalAccessPlanGroups = _PersAccessGroupsViewModel.GetPersAccessGroupByPersNr(0);
        ...
        public void BindLocation()
        {
            ...
            Location Location = new Location() { ID = 0, Name = "keine", Location_Nr = 0 };
            ...
            Location = new Location() { ID = -1, Name = " Alle", Location_Nr = 0 };
        ...
        public void LoadPersonTypes()
        {
            ...
            listPersType.Add(new Pers_Type()
            {
                ID = 0,
                Name = "keine"
            });
        ...
        public void LoadNavPoz()
        {
            ...
            if (!IsCallback)
            {
               ...
               "setTimeout(function() { PageMethods.GetPersonnelByNr(cmbPersName.GetValue(), SetControls) }, 1);", true);
            }
        }
        [WebMethod]
        ...
        public static PersonalData_DTO Getpersonal(string Personalnumber)
        {
            ...
            var persData = _VwPersonnelDataRepository.GetAllInactivePersonnel().Where(x => x.Pers_Nr == Convert.ToInt64(Personalnumber)).FirstOrDefault();
        ...
        {
            if (cmbPersName.Items.FindByValue("0") != null) cmbPersName.Value = "0";
            if (cmbIDNr.Items.FindByValue("0") != null) cmbIDNr.Value = "0";
            if (cmbAusweisNr.Items.FindByValue("0") != null) cmbAusweisNr.Value = "0";
        ...
        {
            cmbPersName.Enabled = drpEnabled;
            cmbIDNr.Enabled = drpEnabled;
            cmbAusweisNr.Enabled = drpEnabled;
            cmbLocations.Enabled = drpEnabled;
            cmbDepartments.Enabled = drpEnabled;
        ...
        protected void SaveAccessPlan(TbAccessPlanPersMapping accessPlan)
        {
            ...
            var location = Convert.ToInt32(cmbLocations.Value);
        ...
        {
            Response.Redirect("PersonalFormInactive.aspx");
        }
        public void BindOtherControls()
        {
            ...
            PersonnelInactiveViewModel _PersonnelViewModel = new PersonnelInactiveViewModel();
            ...
            if (!string.IsNullOrEmpty(Convert.ToString(cmbIDNr.Value)))
            {
                ...
                var vwPer = _VwPersonnelDataRepository.GetAllInactivePersonnel().Where(v => v.Pers_Nr == Convert.ToInt64(cmbIDNr.Value)).FirstOrDefault();
                ...
                {
                    string im = FileProcessor.GetPersonalImageFile(persPassPort.Pers_Passport);
                    if (im != null)
                    {
                        photVal.Value = im;
                        img.Src = im;
        ...
        {
            //cmbPersName.SelectedValue = "0";
            //cmbIDNr.SelectedValue = "0";
            //cmbAusweisNr.SelectedValue = "0";
            //cmbLocations.SelectedValue = "0";
            //cmbDepartments.SelectedValue = "0";
            txtLastName.Text = "";
            txtFirstName.Text = "";
            txtCompany.Text = "";
            txtStreet.Text = "";
        ...
        public List<PersonnelDto> ConvertPersonalsToDtos(List<KruAll.Core.Models.Personal> _Personalslist)
        {
            List<PersonnelDto> _Personals = new List<PersonnelDto>();

            foreach (KruAll.Core.Models.Personal _Personal in _Personalslist)
            {
                PersonnelDto _dto = ConvertPersonalToDto(_Personal);
                _Personals.Add(_dto);
            }
            return _Personals;
        }

        public PersonnelDto ConvertPersonalToDto(KruAll.Core.Models.Personal _Personal)
        {
            KruAll.Core.Models.Pers_Client _pers_client = new PersClientRepository().GetPersClientByPersNo(_Personal.Pers_Nr);
            KruAll.Core.Models.Pers_CostCenters _pers_costCenter = new PersCostCentreRepository().GetCostCenterPersNr(_Personal.Pers_Nr);
            KruAll.Core.Models.Pers_Departments _pers_department = new PersDepartmentRepository().GetPersDepartmentPersNr(_Personal.Pers_Nr);
            KruAll.Core.Models.Pers_AdditionalInfo _pers_additionalInfo = new PersAdditionalInfoRepository().GetPersAdditionalInfoByNr(_Personal.Pers_Nr);
            KruAll.Core.Models.Pers_Locations _pers_location = new PersLocationRepository().GetPercLocationsPersNr(_Personal.Pers_Nr);
            KruAll.Core.Models.Pers_Contact _pers_contact = new PersContactsRepository().GetPersContactByPersNr(_Personal.Pers_Nr);
            KruAll.Core.Models.Pers_Archive _pers_archive = new PersArchiveRepository().GetPersArchiveByPersNr(_Personal.Pers_Nr);
            KruAll.Core.Models.Pers_PinCode _pers_pinCode = new PersPinCodeRepository().GetPinCodeByPersNr(_Personal.Pers_Nr);
            KruAll.Core.Models.Pers_DrivingLicense _pers_drivingLicense = new PersDrivingLicenseRepository().GetPersDrivingLicenseByPersNr(_Personal.Pers_Nr);
            KruAll.Core.Models.Pers_DynamicFields _pers_dynamicField = new PersDynamicFieldsRepository().GetPersDynamicFieldByPersNr(_Personal.Pers_Nr);
            KruAll.Core.Models.Pers_HealthCard _pers_healthCard = new PersHealthCardRepository().GetPersHealthCardByPersNr(_Personal.Pers_Nr);
            KruAll.Core.Models.Pers_IdentityCard _pers_identityCard = new PersIdentityCardRepository().GetPersIdentityCardByPersNr(_Personal.Pers_Nr);
            KruAll.Core.Models.Pers_Passport _pers_passport = new PersPassportRepository().GetPersPassportByPersNr(_Personal.Pers_Nr);
            KruAll.Core.Models.Pers_Photo _pers_photo = new PersImageRepository().GetPersPhotoByPersNr(_Personal.Pers_Nr);
            KruAll.Core.Models.Pers_Transponders _pers_transponder = new PersTranspondersRepository().GetPersTransponderByPersNr(_Personal.Pers_Nr);
            KruAll.Core.Models.Pers_Type _pers_type = new PersonTypeRepository().GetPersonTypeById(_Personal.PersType ?? 0);
            KruAll.Core.Models.Pers_Vehicles _pers_vehicles = new PersVehicleRepository().GetPersVehiclesByPersNr(_Personal.PersType ?? 0);
            KruAll.Core.Models.Pers_Visitors _pers_visitors = new PersVisitorsRepository().GetPersVisitorsByPersNr(_Personal.PersType ?? 0); KruAll.Core.Models.Pers_AccessPlanDuration _pers_accessPlanDuration = new PersAccessPlanDurationRepository().GetPersAccessPlanDurationByPersNr(_Personal.Pers_Nr);

            PersonnelDto _dto = new PersonnelDto();
            //if (_pers_accessPlanDuration != null) { _dto.AccessPlanID = _pers_accessPlanDuration.AccessPlanID; }
            //if (_pers_additionalInfo != null) { _dto.AccountNo = _pers_additionalInfo.AccountNo; }
            //if (_pers_additionalInfo != null) { _dto.AccountOwner = _pers_additionalInfo.AccountOwner; }
            //if (_pers_transponder != null) { _dto.Action = _pers_transponder.Action; }
            if (_Personal != null) { _dto.Active = _Personal.Active; }
            //if (_Personal != null) { _dto.ActiveCardType = _Personal.ActiveCardType; }
            //if (_pers_identityCard != null) { _dto.AdditionalNumber = _pers_identityCard.AdditionalNumber; }
            //if (_pers_additionalInfo != null) { _dto.Additonal = _pers_additionalInfo.Additonal; }
            //if (_pers_additionalInfo != null) { _dto.Address = _pers_additionalInfo.Address; }
            if (_pers_additionalInfo != null) { _dto.ApprovedBy = _pers_additionalInfo.ApprovedBy; }
            if (_pers_archive != null) { _dto.ArchivedBy = _pers_archive.ArchivedBy; }
            if (_pers_archive != null) { _dto.ArchivedDate = _pers_archive.ArchivedDate; }
            if (_pers_additionalInfo != null) { _dto.AuthorisedBy = _pers_additionalInfo.AuthorisedBy; }
            if (_pers_additionalInfo != null) { _dto.BankCode = _pers_additionalInfo.BankCode; }
            if (_pers_additionalInfo != null) { _dto.Bankname = _pers_additionalInfo.Bankname; }
            if (_pers_additionalInfo != null) { _dto.BIC = _pers_additionalInfo.BIC; }
            if (_pers_healthCard != null) { _dto.BoxNumber = _pers_healthCard.BoxNumber; }
            if (_Personal != null) { _dto.Card_Nr = _Personal.Card_Nr; }
            if (_Personal != null) { _dto.Card_Nr1 = _Personal.Card_Nr; }
            if (_Personal != null) { _dto.Card_Nr2 = _Personal.Card_Nr; }
            if (_Personal != null) { _dto.CardActive = _Personal.CardActive; }
            if (_pers_healthCard != null) { _dto.CardNumber = _pers_healthCard.CardNumber; }
            if (_pers_additionalInfo != null) { _dto.CertificateOfEmployment = _pers_additionalInfo.CertificateOfEmployment; }
            if (_pers_additionalInfo != null) { _dto.Children = _pers_additionalInfo.Children; }
            if (_pers_client != null) { _dto.ClientID = _pers_client.ClientID; }
            if (_pers_contact != null) { _dto.CompanyEmail = _pers_contact.CompanyEmail; }
            if (_pers_contact != null) { _dto.CompanyFax = _pers_contact.CompanyFax; }
            if (_pers_contact != null) { _dto.CompanyMobile = _pers_contact.CompanyMobile; }
            if (_pers_additionalInfo != null) { _dto.CompanyName = _pers_additionalInfo.CompanyName; }
            if (_pers_contact != null) { _dto.CompanyTel = _pers_contact.CompanyTel; }
            if (_pers_additionalInfo != null) { _dto.Contract = _pers_additionalInfo.Contract; }
            if (_pers_costCenter != null && _pers_costCenter.CostCenterID != null) { _dto.CostCenterID = int.Parse(String.Format("{0}", _pers_costCenter.CostCenterID)); }
            if (_pers_drivingLicense != null) { _dto.CreatedIn = _pers_drivingLicense.CreatedIn; }
            if (_pers_drivingLicense != null) { _dto.DateOfIssue = _pers_drivingLicense.DateOfIssue; }
            if (_pers_department != null && _pers_department.DepartmentID != null) { _dto.DepartmentID = int.Parse(String.Format("{0}", _pers_department.DepartmentID)); }
            if (_pers_drivingLicense != null) { _dto.DLClass = _pers_drivingLicense.DLClass; }
            if (_pers_drivingLicense != null) { _dto.DLNumber = _pers_drivingLicense.DLNumber; }
            if (_pers_additionalInfo != null) { _dto.DOB = _pers_additionalInfo.DOB; }
            if (_pers_additionalInfo != null) { _dto.DriversLicense = _pers_additionalInfo.DriversLicense; }
            if (_pers_additionalInfo != null) { _dto.EliminatedOn = _pers_additionalInfo.EliminatedOn; }
            if (_pers_additionalInfo != null) { _dto.EmployedAs = _pers_additionalInfo.EmployedAs; }
            if (_pers_additionalInfo != null) { _dto.EmployedFrom = _pers_additionalInfo.EmployedFrom; }
            if (_pers_additionalInfo != null) { _dto.EmploymentType = _pers_additionalInfo.EmploymentType; }
            if (_pers_accessPlanDuration != null) { _dto.EndDate = _pers_accessPlanDuration.EndDate; }
            if (_pers_additionalInfo != null) { _dto.EndOfContract = _pers_additionalInfo.EndOfContract; }
            if (_Personal != null) { _dto.EntryDate = _Personal.EntryDate; }
            if (_Personal != null) { _dto.ExitDate = _Personal.ExitDate; }
            if (_pers_healthCard != null) { _dto.ExpiryDate = _pers_healthCard.ExpiryDate; }
            if (_pers_transponder != null) { _dto.ExtendedTo = _pers_transponder.ExtendedTo; }
            if (_pers_additionalInfo != null) { _dto.EyeColor = _pers_additionalInfo.EyeColor; }
            if (_pers_dynamicField != null) { _dto.FieldIndex = _pers_dynamicField.FieldIndex; }
            if (_pers_dynamicField != null) { _dto.FieldValue = _pers_dynamicField.FieldValue; }
            if (_Personal != null) { _dto.FirstName = _Personal.FirstName; }
            if (_pers_passport != null) { _dto.Gender = _pers_passport.Gender; }
            if (_pers_additionalInfo != null) { _dto.HealInsuaranceNo = _pers_additionalInfo.HealInsuaranceNo; }
            if (_pers_additionalInfo != null) { _dto.HealthInsuarance = _pers_additionalInfo.HealthInsuarance; }
            if (_pers_additionalInfo != null) { _dto.Height = _pers_additionalInfo.Height; }
            if (_pers_additionalInfo != null) { _dto.IBAN = _pers_additionalInfo.IBAN; }
            if (_Personal != null) { _dto.ID = _Personal.ID; }
            if (_pers_identityCard != null) { _dto.IDNumber = _pers_identityCard.IDNumber; }
            if (_Personal != null) { _dto.Imported = _Personal.Imported; }
            if (_pers_drivingLicense != null) { _dto.IssuingAuthority = _pers_drivingLicense.IssuingAuthority; }
            if (_pers_healthCard != null) { _dto.ItemNumber = _pers_healthCard.ItemNumber; }
            if (_Personal != null) { _dto.LastName = _Personal.LastName; }
            if (_pers_additionalInfo != null) { _dto.LearnedOccupation = _pers_additionalInfo.LearnedOccupation; }
            if (_pers_location != null && _pers_location.LocationID != null) { _dto.LocationID = int.Parse(String.Format("{0}", _pers_location.LocationID)); }
            if (_pers_additionalInfo != null) { _dto.MaritalStatus = _pers_additionalInfo.MaritalStatus; }
            if (_Personal != null) { _dto.Memo = _Personal.Memo; }
            if (_Personal != null) { _dto.MiddleName = _Personal.MiddleName; }
            if (_pers_additionalInfo != null) { _dto.Nationality = _pers_additionalInfo.Nationality; }
            if (_pers_additionalInfo != null) { _dto.NoOfHours = _pers_additionalInfo.NoOfHours; }
            if (_pers_additionalInfo != null) { _dto.Passport_Nr = _pers_additionalInfo.Passport_Nr; }
            if (_pers_additionalInfo != null) { _dto.PensionInsuarance = _pers_additionalInfo.PensionInsuarance; }
            if (_Personal != null) { _dto.Pers_Nr = _Personal.Pers_Nr; }
            //if (_pers_photo != null) { _dto.Pers_Passport = string.IsNullOrWhiteSpace(_pers_photo.Pers_Passport) ? _pers_photo.Pers_Passport : FileProcessor.GetMemberImageFile(_pers_photo.Pers_Passport); }
            if (_pers_photo != null) { _dto.PassPhoto = string.IsNullOrWhiteSpace(_pers_photo.Pers_Passport) ? _pers_photo.Pers_Passport : FileProcessor.GetPersonalImageFile(_pers_photo.Pers_Passport); }

            if (_Personal != null) { _dto.PersType = _Personal.PersType; }
            if (_pers_type != null) { _dto.PersTypeColor = _pers_type.PersTypeColor; }
            if (_pers_additionalInfo != null) { _dto.PhysicalAddress = _pers_additionalInfo.PhysicalAddress; }
            if (_pers_pinCode != null && _pers_pinCode.PinCode != null) { _dto.PinCode = _pers_pinCode.PinCode; }
            if (_pers_pinCode != null && _pers_pinCode.PinCode != null) { _dto.PinCode1 = _pers_pinCode.PinCode; }
            if (_pers_pinCode != null && _pers_pinCode.PinCodeStatus != null) { _dto.PinCodeStatus = _pers_pinCode.PinCodeStatus; }
            if (_pers_pinCode != null && _pers_pinCode.PinCodeType != null) { _dto.PinCodeType = _pers_pinCode.PinCodeType; }
            if (_pers_additionalInfo != null) { _dto.PlaceOfBirth = _pers_additionalInfo.PlaceOfBirth; }
            if (_pers_additionalInfo != null) { _dto.Position = _pers_additionalInfo.Position; }
            if (_pers_contact != null) { _dto.PostalAddress = _pers_contact.PostalAddress; }
            if (_Personal != null) { _dto.PostalCode = _Personal.PostalCode; }
            if (_pers_passport != null) { _dto.PPNumber = _pers_passport.PPNumber; }
            if (_pers_contact != null) { _dto.PrivateEmail = _pers_contact.PrivateEmail; }
            if (_pers_contact != null) { _dto.PrivateFax = _pers_contact.PrivateFax; }
            if (_pers_contact != null) { _dto.PrivateMobile = _pers_contact.PrivateMobile; }
            if (_pers_contact != null) { _dto.PrivateTel = _pers_contact.PrivateTel; }
            if (_pers_additionalInfo != null) { _dto.Profession = _pers_additionalInfo.Profession; }
            if (_pers_additionalInfo != null) { _dto.Reason = _pers_additionalInfo.Reason; }
            if (_pers_additionalInfo != null) { _dto.ResidencePermit = _pers_additionalInfo.ResidencePermit; }
            if (_pers_additionalInfo != null) { _dto.Salutation = _pers_additionalInfo.Salutation; }
            if (_pers_healthCard != null) { _dto.SecurityNumber = _pers_healthCard.SecurityNumber; }
            if (_pers_additionalInfo != null) { _dto.Since = _pers_additionalInfo.Since; }
            if (_pers_additionalInfo != null) { _dto.SozVerzNo = _pers_additionalInfo.SozVerzNo; }
            if (_pers_accessPlanDuration != null) { _dto.StartDate = _pers_accessPlanDuration.StartDate; }
            if (_pers_additionalInfo != null) { _dto.Street = _pers_additionalInfo.Street; }
            if (_Personal != null) { _dto.StreetNr = _Personal.StreetNr; }
            if (_pers_additionalInfo != null) { _dto.TaxClass = _pers_additionalInfo.TaxClass; }
            if (_pers_additionalInfo != null) { _dto.TaxOfficee = _pers_additionalInfo.TaxOfficee; }
            if (_Personal != null) { _dto.Title = _Personal.Title; }
            if (_pers_transponder != null) { _dto.TransponderDeactivatedOn = _pers_transponder.TransponderDeactivatedOn; }
            if (_pers_transponder != null) { _dto.TransponderNr = _pers_transponder.TransponderNr; }
            if (_pers_transponder != null) { _dto.TransponderStatus = _pers_transponder.TransponderStatus; }
            if (_pers_transponder != null) { _dto.TransponderType = _pers_transponder.TransponderType; }
            if (_pers_transponder != null) { _dto.ValidFrom = _pers_transponder.ValidFrom; }
            if (_pers_transponder != null) { _dto.ValidTo = _pers_transponder.ValidTo; }
            if (_pers_vehicles != null) { _dto.VehicleID = _pers_vehicles.VehicleID; }
            if (_pers_additionalInfo != null) { _dto.VehicleModel = _pers_additionalInfo.VehicleModel; }
            if (_pers_additionalInfo != null) { _dto.VehicleReg = _pers_additionalInfo.VehicleReg; }
            //if (_Personal != null) { _dto.VehicleType = _Personal.VehicleType; }
            if (_pers_visitors != null) { _dto.VisitorID = _pers_visitors.VisitorID; }
        ...
        [System.Web.Services.WebMethod(MessageName = "UpdatePersonalInactiveStatus")]
        [System.Web.Script.Services.ScriptMethod()]
        public static KruAll.Core.Models.Personal UpdatePersonalInactiveStatus(PersonnelDto dto)
        {
            try
            {
                KruAll.Core.Models.Personal PersonalExist = new KruAll.Core.Models.Personal();
                PersonnelInactiveRepository rep = new PersonnelInactiveRepository();
                Personal Personalfrm = new Personal();

                PersonalExist = rep.GetAllInactivePersonnel().FirstOrDefault(x => x.Pers_Nr == Convert.ToInt32(dto.Pers_Nr));
                if (PersonalExist != null)
                {
                    PersonalExist.Active = dto.Active ?? false;

                    rep.EditPersonnel(PersonalExist);
                }
        ...
        [WebMethod]
        public static PersonnelDto GetPersonnelByNr(int nr)
        {
            PersonnelDto _dto = new PersonnelDto();
            PersonalInactive _prsnlfrm = new PersonalInactive();
            KruAll.Core.Models.Personal _Personal = new PersonnelRepository().GetPersonnelByPersNumber(nr);
            if (_Personal == null)
            {
                return null;
            }
            _dto = _prsnlfrm.ConvertPersonalToDto(_Personal);
            return _dto;
        }
        ...
        {
            if (PersonnelList.Count == 0) PersonnelList = personalViewModel.GetAllInActivePersonnel();
