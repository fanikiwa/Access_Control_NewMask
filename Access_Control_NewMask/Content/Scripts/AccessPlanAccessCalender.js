...
$(function () {
    ...
    //            case "0":
    //            case "1":
    //                document.location.href = "AccessPlan.aspx";
    //                break;
    ...
    //            case "3":
    //                var detectedChanges = $("#hiddenFieldDetectChanges").attr("value");
    ...
    $("#btnSaveAccessPlan").click(function (evt) {
        evt.preventDefault();
        SaveAccessPlan();
    });
    $("#btnNew").click(
...
function showAccessProfileSearchTable() {
    ...
    $("#btnBack").bind("click", function (evt) {
        ...
        if (mode === "1") {
            ...
            document.location.href = "AccessPlanAccessCalender.aspx";
        }
        $("#btnBack").unbind("click");
        $("#btnBack").bind("click", function (evt) { defaultBackFx(evt) });
    });
...
function getLocalizedText(key) {
    ...
    $.ajax({
        type: "POST",
        async: false,
        url: "AccessPlanAccessCalender.aspx/GetLocalizedText",
...
function OnCellClick(cell, rowIndex) {
    ...
    if (fieldName !== "MonthName") {
        ...
        $.ajax({
            type: "POST",
            async: false,
            url: "AccessPlanAccessCalender.aspx/GetAccessProfilesByMonth",
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            data: JSON.stringify(data),
            success: function () {
...
function GetDailyScheduleValues() {
    ...
    $.ajax({
        type: "POST",
        url: "AccessPlanAccessCalender.aspx/UpdateAccessCalenderSchedule",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        data: JSON.stringify(data),
        success: function () {
...
function EnableDisableControls() {
    ...
    document.getElementById("btnAccessProfile").disabled = true;
...
function DeleteButtonConfirm() {
    ...
    var box_content = '<div id="overlay"><div id="box_flame"><div id="dialogBox"> <div id="prompttopclose"><label id="lbltitletop">Zutritts</label><button id="promptbtnclose"  onclick="resetImportantInfoDialogDiv(); return false;" /></div><div id="areasavepop"><img src="../../Images/FormImages/stop-save1-01.png" alt="Stop" class="stopPic" height="150" width="150" align="middle"><div id="lbltitlemsg">' + message + '</div> <button id="btnOkDelete"  style="margin-left: 27%; margin-right: 0px;"  onclick="DeleteAccessCalendarCustom()"></button><button id="btnCancelDelete"  onclick="resetImportantInfoDialogDiv(); return false;" style=" position: relative;"></button></div></div></div></div>';
...
function DeleteAccessCalendarCustom() {
    ...
    PageMethods.DeleteAccessCalendar(id, OnDelete_CallBack);
}
...
function OnGetNumberBack_CallBack(responce) {
    ...
    if (accessPlanId === 0) {
        ...
        PageMethods.SaveAccessCalendar(mode, accessPlanId, accessCalenderNr, accessPlanName, calendarDate, $("#txtMemo").val(), OnSaveBack_CallBack);
    }
...
function SaveAccessCalendar() {
    ...
    $.ajax({
        type: "POST",
        async: false,
        url: "AccessPlanAccessCalender.aspx/SaveAccessCalendar",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
...
function CancelOnBackButton() {
    resetImportantInfoDialogDiv();
    //document.location.href = "/Content/AccessPlanAccessCalender.aspx";
}
...
function OnGetNumberInsertEdit_CallBack(responce) {
    ...
    if (accessPlanId === 0) {
        ...
        PageMethods.SaveAccessCalendar(mode, accessPlanId, accessCalenderNr, accessPlanName, calendarDate, $("#txtMemo").val(), OnSaveCalendar_CallBack);
    }
    else if (accessPlanId > 0) {
        $("#hiddenFieldFormMode").attr("value", "3");
        var mode = 3;
