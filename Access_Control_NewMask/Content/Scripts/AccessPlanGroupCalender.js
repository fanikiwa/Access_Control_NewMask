...
function defaultBackFx(evt) {
    ...
    switch (detectedChanges) {
        case "0":
            ...
            document.location.href = "AccessPlanGroup.aspx";
            break;
...
function OnGetNumberBack_CallBack(responce) {
    ...
    var groupId = $('#txtAccessGroupId').val();
    if ((calendarYear === NaN) || (calendarYear === 0)) {
        calendarYear = new Date().getFullYear();
    }
    ...
    if (accessPlanId === 0) {
        ...
        PageMethods.SaveAccessCalendar(mode, accessPlanId, accessCalenderNr, accessPlanName, calendarDate, $("#txtMemo").val(), groupId, OnSaveBack_CallBack);
    }
...
function OnGetNumberInsertEdit_CallBack(responce) {
    ...
    if (accessPlanId === 0) {
        ...
        PageMethods.SaveAccessCalendar(mode, accessPlanId, accessCalenderNr, accessPlanName, calendarDate, $("#txtMemo").val(), groupId, OnSaveCalendar_CallBack);
    }
    else if (accessPlanId > 0) {
        $("#hiddenFieldFormMode").attr("value", "3");
        var mode = 3;
