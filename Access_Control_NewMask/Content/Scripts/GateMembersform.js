...
$(function () {
    ...
    $('#btnPERSDOC').on("click", function (e) {
        e.preventDefault();
        document.location.href = "/Content/GateMembersDocumente.aspx";
        var memberId = parseInt(cobMemberNr.GetValue());
        if (memberId > 0) {
    ...
    $('#btnPersonalStamm').on("click", function (e) {
        e.preventDefault();
        var memberId = parseInt(cobMemberNr.GetValue());
        document.location.href = "/Content/GateMembers.aspx?Id=" + cobMemberNr.GetValue();
    });
    ...
    //   $('#btnPersonalForm').removeClass('newstandardbutton');
});
...
function getLocalizedText(key) {
    ...
    $.ajax({
        type: "POST",
        async: false,
        url: "GateMembersform.aspx/GetLocalizedText",
