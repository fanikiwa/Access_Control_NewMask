...
$(document).ready(function () {
    ...
    $("#btnMemberForm").click(function (ev) {
        ...
        if (memberId > 0) {
            document.location.href = "/Content/Membersform.aspx?Id=" + cobMemberNr.GetValue();
        } else {
            document.location.href = "/Content/Membersform.aspx?Id=" + 0;
        }
    });
    ...
    $("#fvNavPrev").click(function (evt) {
        ...
        var personalid = cobMemberNr.GetValue();
        console.log(personalid);
        PageMethods.GetMemberByID(personalid, OnGetMemberByID_CallBack);
    });
...
function OnSaveMemberDetails_CallBack(id) {
    var _id = id;
    if ($("#chkActivateMember").is(":checked")) _id = 0;
...
function OnSaveMemberONBackDetails_CallBack(id) {
    ...
    if (saveTransponders === true) {
        ...
        //setTimeout(function (ev) {  }, 1);
        //setTimeout(function (ev) { GoToIndex() }, 1);
        //window.location.href = "/Index.aspx";
    }
...
function cobMemberNameEndCallBack(s, e) {
    ...
    if (saveTransponders === true) {
        ...
        //    PageMethods.SaveMemberAccessGroups(cobMemberName.GetValue(), JSON.stringify(GetAccessGroupUpdate()),
        //        function (resp) {
        //            setTimeout(function (ev) { grdAccessGroups.PerformCallback(cobMemberName.GetValue()) }, 1);
        //        },
        //        function (err) {
        //            console.log(err);
        //        });
        ...
        PageMethods.SaveMemberAccessGroups(cobMemberName.GetValue(), JSON.stringify(GetAccessGroupUpdate()),
            SaveMemberAccessGroups_AfterSaveCallBack,
            function (err) {
                console.log(err);
            });
    }
...
function SaveMemberAccessGroups_AfterSaveCallBack() {

    grdAccessGroups.PerformCallback(cobMemberName.GetValue());
}
...
function OnGetMemberByID_CallBack(member) {
    if (member !== null) {
        ...
        setTimeout(function (ev) { grdAccessGroups.PerformCallback(member.ID) }, 1);
    }
    else {
        ClearControls();
...
function RedirectContactDuration() {
    ...
    var pageFrom = "/Content/Members.aspx";
...
function getLocalizedText(key) {
    ...
    $.ajax({
        type: "POST",
        async: false,
        url: "Members.aspx/GetLocalizedText",
