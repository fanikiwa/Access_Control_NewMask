...
{
    public partial class VisitorPlanVisitorCalender : BasePage
    {
        enum FormMode
        {
            None,
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (hiddenFieldDetectChanges.Value != null)
            {
                ...
                {
                    //Response.Redirect("VisitorPlan.aspx");
                }
            }
        ...
        void DisplayVisitorPlanCalendar()
        {
            ...
            int.TryParse(Request.QueryString["profileId"], out visitorPlanId);
            var visitorPlan = new VisitorPlanViewModels().GetVisitorPlanByID(Convert.ToInt64(visitorPlanId));

            if (visitorPlan == null) return;

            if (!visitorPlan.AccessCalendarId.HasValue) return;

            Session["VisitorPlan"] = visitorPlan;

            _accessCalendarId = (int)visitorPlan.AccessCalendarId.Value;
        ...
        static void MapVisitorPlanCalendar(AccessCalendar accessCalendar, long visitorPlanId)
        {

            if (accessCalendar == null) return;
            var visitorPlan = new VisitorPlanViewModels().GetVisitorPlanByID(visitorPlanId);

            if (visitorPlan == null) return;

            var visitorPlanToEdit = new TbVisitorPlan
            {
                ID = visitorPlan.ID,
                VisitorPlanNr = visitorPlan.VisitorPlanNr,
                VisitorPlanDescription = visitorPlan.VisitorPlanDescription,
                AccessCalendarId = accessCalendar.ID,
                BuildingPlanID = visitorPlan.BuildingPlanID,
                HolidayPlanCalendarId = visitorPlan.HolidayPlanCalendarId
            };

            new VisitorPlanViewModels().UpdateVisitorPlan(visitorPlanToEdit);
        }
        ...
        [WebMethod]
        public static void DeleteAccessCalendar(long id, long visitorPLanId)
        {
        ...
        protected void GetPassedData()
        {
            ...
            var visitorPLan = new VisitorPlanViewModels().GetVisitorPlanByID(Convert.ToInt64(id));
            if(visitorPLan == null)
            {
                Response.Redirect("~/Content/VisitorPlan.aspx");
            }
            txtVisitorPlanNumber.Text = visitorPLan.VisitorPlanNr.ToString();
            txtVisitorPlanName.Text = visitorPLan.VisitorPlanDescription.ToString();
            txtVisitorPlanId.Text = visitorPLan.ID.ToString();
            Session["visitorProfileId"] = id;
        }
        ...
        [WebMethod]
        public static long SaveAccessCalendar(int formMode, long calenderId, int calendarNumber, string calendarName, DateTime calendarDate, string memo, long visitorPLanId)
        {
            long Id = 0;
            ...
            if (accessCalendar != null)
            {
                ...
                MapVisitorPlanCalendar(accessCalendar, visitorPLanId);
            }
