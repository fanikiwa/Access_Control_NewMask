...
namespace Access_Control_NewMask.ViewModels
{
    public class MembersViewModel
    {
        ...
        public MembersData SaveMemberData(MembersDataDto member)
        {
            ...
            if (memberData.ID > 0)
            {
                new MembersDataRepository().EditMemberData(memberData);
                if (memberData != null)
                {
            ...
            {
                new MembersDataRepository().NewMemberData(memberData);
                if (memberData != null)
                {
                    CreateAccessPlan(member, memberData);
                }
        ...
        public List<MembersDataDto> MemberDetails(bool active)
        {
            List<MembersDataDto> membersList = new List<MembersDataDto>();
            var members = new MembersDataRepository().GetAllMembersData(active);
            foreach (var member in members)
            {
                MembersDataDto memberDto = new MembersDataDto()
                {
        ...
        public List<MembersDataDto> GetAllMembersData(bool active)
        {
            List<MembersDataDto> membersList = new List<MembersDataDto>();
            var members = new MembersDataRepository().GetAllMembersData(active);
            foreach (var member in members)
        ...
        public List<AccessPlanMembersDto> GetMembersForAccessPlanList(long accessPlan_Id)
        {
            ...
            var accessplannedmemberids = new MembersAccessPlanMappingRepository().GetAllMembersPLans().Where(x => x.AccessPlan_Nr == accessPlan_Id).Select(x => Convert.ToInt64(x.MemberID)).ToList();


            var accessplannedmemberdetails = from apmd in new MembersDataRepository().GetAllMembersData()
        ...
        public MembersDataDto GetMemberById(long id)
        {
            ...
            if (member != null)
            {
                ...
                memberDto.Active = member.Active ?? false;
            }
            return memberDto;
        }
        public MembersDataDto GetMemberById(long id, bool active)
        {
            MembersDataDto memberDto = new MembersDataDto();
            var member = new MembersDataRepository().GetMemberDataById(id, active);
            if (member != null)
            {
                memberDto.ID = member.ID;
                memberDto.SurName = member.SurName;
        ...
        public List<MemberSearchDto> MemberSearchDetails()
        {
            ...
            foreach (var member in members)
            {
                ...
                memberData.Active = member.Active ?? false;
